...
def explain_prediction(text, model, tokenizer, class_names, device):
    ...
    # Get prediction
    with torch.no_grad():
        # Renamed to outputs_obj to avoid confusion
        outputs_obj = model(**inputs)

    probabilities = F.softmax(outputs_obj.logits, dim=-1)
    predicted_class_idx = torch.argmax(probabilities, dim=1).item()
    predicted_class = class_names[predicted_class_idx]

    # Define a forward function for Captum that returns logits
    # This wrapper ensures that Captum receives a tensor (logits) from the model's forward pass.
    def model_forward_for_captum(input_ids_captum, attention_mask_captum=None):
        # 'model' is captured from the outer scope of explain_prediction
        output = model(input_ids=input_ids_captum,
                       attention_mask=attention_mask_captum)
        return output.logits

    # Setup for attribution
    # Pass the wrapper function to LayerIntegratedGradients
    lig = LayerIntegratedGradients(
        model_forward_for_captum, model.roberta.embeddings)

    # Get attributions
    # Ensure attention_mask is passed via additional_forward_args
    # The 'inputs' argument to attribute should be the tensor for input_ids
    # The 'baselines' should match the shape of input_ids
    attributions, delta = lig.attribute(
        inputs=inputs['input_ids'],
        baselines=torch.zeros_like(inputs['input_ids']),
        # Pass attention_mask as a tuple
        additional_forward_args=(inputs['attention_mask'],),
        target=predicted_class_idx,
        return_convergence_delta=True,
        n_steps=50  # Using a moderate number of steps for efficiency
    )
...
def train_model_with_cv():
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(df, df['label'])):
        ...
        num_training_steps = len(
            train_dataset) * NUM_EPOCHS // (BATCH_SIZE * GRADIENT_ACCUMULATION_STEPS)
        ...
        training_history['train_loss'].append(train_result.training_loss)
        training_history['eval_loss'].append(eval_result['eval_loss'])
        training_history['eval_accuracy'].append(eval_result['eval_accuracy'])
        training_history['eval_f1'].append(eval_result['eval_f1'])
    ...
    for metric in fold_results[0].keys():
        avg_results[metric] = sum(fold[metric]
                                  for fold in fold_results) / len(fold_results)
...
if __name__ == "__main__":
    ...
    _, test_df = train_test_split(
        test_df, test_size=0.2, stratify=test_df['label'], random_state=SEED)
    ...
    for cls in class_names:
        print(
            f"{cls}: F1={test_report[cls]['f1-score']:.4f}, Precision={test_report[cls]['precision']:.4f}, Recall={test_report[cls]['recall']:.4f}")
    ...
    explanation = explain_prediction(
        example_text, model, tokenizer, class_names, device)
...
Authentic: F1=0.7941, Precision=0.8438, Recall=0.7500
Generic: F1=0.6800, Precision=0.6071, Recall=0.7727

Overall: F1=0.7780, Accuracy=0.7714
"""